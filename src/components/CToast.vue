<template>
  <div
    ...
    class="w-full max-w-sm transform rounded-lg p-4 shadow-lg transition-all duration-300 ease-in-out"
    role="alert"
    @click="close"
  >
    <div class="flex items-start gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p v-if="title" class="text-primary-700 text-sm font-medium">{{ title }}</p>
        <p class="text-primary-600 text-sm" :class="{ 'mt-1': title }">{{ message }}</p>
      </div>

      <!-- Close button -->
      <button v-if="closable" @click.stop="close" :class="closeButtonClasses" aria-label="Close">
