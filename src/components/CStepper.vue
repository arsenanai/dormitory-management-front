<template>
  <div class="flex w-full flex-1 flex-col items-stretch justify-between gap-4">
    <!-- Progress Bar -->
    <div class="relative pt-1">
      <div class="mb-2 flex items-center justify-center">
        <span class="text-primary-700 text-lg font-semibold">
          {{ currentStepTitle }}
        </span>
      </div>
      <div class="bg-primary-200 flex h-2 overflow-hidden rounded text-xs">
        <div
          :style="{ width: progressPercentage + '%' }"
          class="bg-primary-500 flex flex-col justify-center text-center whitespace-nowrap text-white shadow-none transition-all duration-500"
        ></div>
      </div>
    </div>

    <!-- Step Content -->
    <div class="step-content justify-items-between flex flex-auto flex-col items-stretch">
      <slot />
    </div>
