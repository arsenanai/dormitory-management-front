...
export const useDormitoriesStore = defineStore("dormitoriesStore", {
  ...
  actions: {
    ...
    filterByCapacity(minCapacity: number, maxCapacity: number): Dormitory[] {
      return this.dormitories.filter(
        (dorm) => (dorm.capacity || 0) >= minCapacity && (dorm.capacity || 0) <= maxCapacity
      );
    },
    ...
    setSelectedDormitory(dormitory: Dormitory) {
      ...
      localStorage.setItem(
        LOCAL_STORAGE_SELECTED_DORMITORY_KEY,
        JSON.stringify(this.selectedDormitory)
      );
    },
