            ...
            <section>
                ...
                <form @submit.prevent="saveAllSettings" class="space-y-6">
                    ...
                    <div class="border border-gray-200 rounded-lg p-4">
                        ...
                        <CSelect id="currency-symbol" v-model="dormitorySettingsForm.currency_symbol"
                            :label="t('Currency')" class="mb-4" :options="currencyOptions" required />
                    </div>

                    <!-- Dormitory Settings -->
                    <div class="border border-gray-200 rounded-lg p-4">
                        <h3 class="text-md font-medium mb-3 text-primary-600">{{ t('Dormitory Settings') }}</h3>
                        <CTextarea id="dormitory-rules" v-model="dormitorySettingsForm.dormitory_rules"
                            :label="t('Dormitory Rules and Regulations')" class="mb-4" rows="10" />
                    </div>

                    <!-- Dormitory Bank requisites/payment instructions -->
                    <div class="border border-gray-200 rounded-lg p-4">
                        <h3 class="text-md font-medium mb-3 text-primary-600">{{ t('Bank requisites / payment instructions') }}</h3>
                        <CTextarea id="dormitory-rules" v-model="dormitorySettingsForm.bank_requisites"
                            :label="t('Dormitory Bank requisites / payment instructions')" class="mb-4" rows="10" />
                    </div>

                    <!-- 1C Integration Toggle -->
                    <div class="border border-gray-200 rounded-lg p-4">
                        <h3 class="text-md font-medium mb-3 text-primary-600">{{ t('1C Integration') }}</h3>
                        <CCheckbox id="onec-enabled" v-model="onecForm.onec_enabled"
                        ...
                        <div v-if="onecForm.onec_enabled" class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <CInput v-model="onecForm.onec_password" :label="t('1C Password')" type="password"
                                required />
                            <CInput v-model="onecForm.onec_sync_interval" :label="t('Sync Interval (seconds)')"
                    ...
                    <div class="border border-gray-200 rounded-lg p-4">
                        ...
                        <CCheckbox id="kaspi-enabled" v-model="kaspiForm.kaspi_enabled"
                    ...
                    <div class="border border-gray-200 rounded-lg p-4">
                        ...
                        <div v-if="cardReaderForm.card_reader_enabled"
                            ...
                            <CInput v-model="cardReaderForm.card_reader_port" :label="t('Card Reader Port')"
                                type="number" required />
                            <CInput v-model="cardReaderForm.card_reader_timeout" :label="t('Timeout (seconds)')"
                                type="number" required />
                        </div>
                        <div v-if="cardReaderForm.card_reader_enabled" class="mt-3">
                            ...
                            <div class="space-y-2">
                                <div v-for="(location, index) in cardReaderForm.card_reader_locations" :key="index"
                                    class="flex gap-2">
                                    <CInput v-model="cardReaderForm.card_reader_locations[index]"
                                        :placeholder="t('Location name')" class="flex-1" />
...
const kaspiForm = reactive<KaspiSettings>({ kaspi_enabled: false, kaspi_api_key: null, kaspi_merchant_id: null, kaspi_webhook_url: null });
const cardReaderForm = reactive<CardReaderSettings>({ card_reader_enabled: false, card_reader_host: '', card_reader_port: 8080, card_reader_timeout: 30, card_reader_locations: ['Main Entrance'] });
const onecForm = reactive<OneCSettings>({ onec_enabled: false, onec_host: '', onec_database: '', onec_username: '', onec_password: '', onec_sync_interval: 3600 });
...
const loadSettings = async () => {
    ...
    if (settingsStore.cardReaderSettings) Object.assign(cardReaderForm, settingsStore.cardReaderSettings);
    ...
    if (settingsStore.publicSettings) Object.assign(dormitorySettingsForm, settingsStore.publicSettings);
...
const saveAllSettings = async () => {
    try {
        ...
        const cardReaderPayload = {
            ...
            card_reader_locations: Array.isArray(cardReaderForm.card_reader_locations) ? cardReaderForm.card_reader_locations : [],
        };
