<template>
  <div class="flex min-h-screen items-center justify-center bg-primary-50">
    <div class="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
      <h2 class="text-2xl font-bold text-center text-primary-700">{{ t('Set New Password') }}</h2>
      <form v-if="!successMessage && !tokenError" @submit.prevent="onSubmit" class="flex flex-col gap-4">
        <CInput
          id="new-password"
          v-model="password"
          type="password"
      ...
      <div v-if="successMessage" class="text-green-600 text-center p-4 bg-green-50 rounded">
        {{ successMessage }}
        <router-link to="/" class="block mt-4 text-primary-600 hover:underline">{{ t('Go to Login') }}</router-link>
      </div>
      <div v-if="tokenError" class="text-red-600 text-center p-4 bg-red-50 rounded">
        {{ tokenError }}
        <router-link to="/password-reset" class="block mt-4 text-primary-600 hover:underline">{{ t('Request a new link') }}</router-link>
      </div>
    </div>
  </div>
</template>
