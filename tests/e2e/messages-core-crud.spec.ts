...
test.describe('Messages Index Page Core Functionality', () => {
  test('should perform core CRUD operations on messages', async ({ page }) => {
    ...
    const newMessageExists = await page.locator('text=Test Message from E2E').first().isVisible();
    expect(newMessageExists).toBe(true);
    
    // 4. TEST DELETING A MESSAGE
    console.log('=== 4. TESTING DELETING A MESSAGE ===');
    ...
    console.log('=== CORE CRUD TESTS PASSED ===');
  });
});
